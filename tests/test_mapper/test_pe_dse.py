...
def test_harris():
    PE = {"fc": lassen_fc, "pe_fc_name": "WrappedPE", "op": lassen_asm, "rules": None}
    generate_testbench(PE, "final_harris", do_mapping=False)


def generate_testbench(PE, app, do_mapping=True):
    arch_fc = PE["fc"]
    ...
    name = putil.load_from_peak(CoreIRNodes, arch_fc)
    m = CoreIRNodes.coreir_modules[name]
    m.print_()
